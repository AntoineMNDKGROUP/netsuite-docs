...
def extract_scripts(
    ...
    rows = _try_variants(suiteql, SCRIPTS_QUERY_VARIANTS, limit)
    ...
    # Détection des suppressions (uniquement sur run complet, pas en --limit
    # qui ne remonte qu'une partie des scripts)
    if not limit:
...
def extract_script_deployments(
    ...
    rows = _try_variants(suiteql, DEPLOYMENTS_QUERY_VARIANTS, limit)
